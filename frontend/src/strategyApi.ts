...
async function request<T>(path: string, init: RequestInit = {}, fetcher: typeof fetch = fetch): Promise<T> {
  ...
  let payload: { detail?: StrategyApiDetail | string } = {};
  try { payload = (await response.json()) as typeof payload; } catch { /* unavailable or malformed response */ }
  const raw = payload.detail;
  const detail = raw && typeof raw === "object" ? raw : { code: "request_failed", message: typeof raw === "string" ? raw : `Request failed (${response.status})` };
...
export const strategyApi = {
  ...
  create: (name: string, canonical: CanonicalStrategyV1, fetcher?: typeof fetch) => request<StrategyDetail>("/v1/strategies", json("POST", { name, canonical_strategy: canonical }), fetcher),
  rename: (id: string, name: string, fetcher?: typeof fetch) => request<StrategyDetail>(`/v1/strategies/${encodeURIComponent(id)}`, json("PATCH", { name }), fetcher),
  archive: (id: string, fetcher?: typeof fetch) => request<void>(`/v1/strategies/${encodeURIComponent(id)}`, { method: "DELETE" }, fetcher),
  save: (id: string, parentId: string, canonical: CanonicalStrategyV1, fetcher?: typeof fetch) => request<SaveRevisionResponse>(`/v1/strategies/${encodeURIComponent(id)}/revisions`, json("POST", { expected_parent_revision_id: parentId, canonical_strategy: canonical }), fetcher),
