...
import { filterResultEvents, projectResultEvents, type ResultEventFilter, type ResultEventPresentation } from "../domain/resultEvents";
...
export function ResultWorkspace({ response, run, strategyName, onBack, researchContext, onResearchContextChange, onShowInStrategy, onViewInFlow, onComparisonReady }: Props) {
  ...
  const [selectedEvent, setSelectedEvent] = useState<ResultEventPresentation | null>(null);
  const [eventFilter, setEventFilter] = useState<ResultEventFilter>("all");
  const analysisRef = useRef<HTMLDivElement>(null);
  const events = useMemo(() => run ? projectResultEvents(run.id, summaries) : [], [run?.id, summaries]);
  const filteredEvents = useMemo(() => filterResultEvents(events, eventFilter), [events, eventFilter]);
  useEffect(() => { setSummaries([]); setSelectedEvent(null); setEventFilter("all"); if (run?.status !== "succeeded") { setEvidenceState("loaded"); return; } let cancelled = false; setEvidenceState("loading"); decisionEvidenceApi.list(run.id).then(({ items }) => { if (!cancelled) { setSummaries(items); setEvidenceState("loaded"); } }).catch(() => { if (!cancelled) setEvidenceState("error"); }); return () => { cancelled = true; }; }, [run?.id, run?.status]);
  useEffect(() => { if (!run || !researchContext || researchContext.runId !== run.id || selectedEvent || events.length === 0) return; const restored = events.find((event) => researchContext.decisionId ? event.decisionIds.includes(researchContext.decisionId) : event.sessionId === researchContext.sessionId); if (restored) { setSelectedEvent(restored); requestAnimationFrame(() => analysisRef.current?.scrollIntoView({ block: "start" })); } }, [researchContext, run, selectedEvent, events]);
  function selectDecision(event: ResultEventPresentation, reveal = false) { setSelectedEvent(event); if (run) onResearchContextChange?.({ runId: run.id, decisionId: event.decisionId, sessionId: event.sessionId, asset: null }); if (reveal) requestAnimationFrame(() => analysisRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })); }
  function changeFilter(filter: ResultEventFilter) { setEventFilter(filter); if (selectedEvent && filter !== "all" && selectedEvent.category !== filter) setSelectedEvent(null); }
  ...
  return <section className="result-workspace page"><header className="workspace-heading"><div><span className="eyebrow">{persistent ? "Saved test" : "Test of current changes"}</span><h1>{strategyName}</h1>{config && <p>{config.start_date} to {config.end_date}</p>}<span className={`run-kind ${persistent ? "persistent" : "temporary"}`}>{persistent ? "Saved in Backtests" : "Temporary result"}</span></div><button className="secondary-button" onClick={onBack}>Back</button></header>
    ...
    {result && <BacktestResultPanel result={result} selectedDecisionId={selectedEvent?.decisionId} allEvents={events} events={filteredEvents} filter={eventFilter} onFilter={persistent ? changeFilter : undefined} onSelectDecision={(event) => selectDecision(event, true)} />}
    ...
    {run?.status === "succeeded" ? <div ref={analysisRef}><DecisionAnalysis runId={run.id} events={filteredEvents} allEventCount={events.length} listState={evidenceState} selected={selectedEvent} selectedAsset={researchContext?.runId === run.id && researchContext.decisionId === selectedEvent?.decisionId ? researchContext.asset : null} onSelect={selectDecision} onSelectAsset={(asset) => { if (selectedEvent) onResearchContextChange?.({ runId: run.id, decisionId: selectedEvent.decisionId, sessionId: selectedEvent.sessionId, asset }); }} onShowInStrategy={onShowInStrategy ? (componentId, fieldPath, asset) => onShowInStrategy(run.revision_id, componentId, fieldPath, { runId: run.id, decisionId: selectedEvent!.decisionId, sessionId: selectedEvent!.sessionId, asset }) : undefined} onViewInFlow={onViewInFlow ? (componentId, fieldPath, asset) => onViewInFlow(run.revision_id, componentId, fieldPath, { runId: run.id, decisionId: selectedEvent!.decisionId, sessionId: selectedEvent!.sessionId, asset }) : undefined} onComparisonReady={onComparisonReady} /></div> : <section className="analysis-boundary"><div className="analysis-icon">↳</div><div><span className="eyebrow">Analysis</span><h2>Decision analysis is unavailable</h2><p>{run ? "This run did not complete with a result and decision evidence." : "Save this strategy and create a saved test to inspect its decisions."}</p></div></section>}
  </section>;
}
