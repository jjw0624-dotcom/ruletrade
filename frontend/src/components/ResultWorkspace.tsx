...
export function ResultWorkspace({ response, run, strategyName, onBack, onShowInStrategy }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false; setEvidenceState("loading"); setSummaries([]); setSelectedSession(null); setSelectedTimestamp(null);
    decisionEvidenceApi.list(run.id).then(({ items }) => { if (!cancelled) { setSummaries(items); setEvidenceState("loaded"); } }).catch(() => { if (!cancelled) setEvidenceState("error"); });
    return () => { cancelled = true; };
  }, [run?.id, run?.status]);
  function selectDecision(session: DecisionSession, reveal = false) { setSelectedSession(session); setSelectedTimestamp(session.sessionId); if (reveal) requestAnimationFrame(() => analysisRef.current?.scrollIntoView({ behavior: "smooth", block: "start" })); }
  ...
  return <section className="result-workspace page"><header className="workspace-heading"><div><span className="eyebrow">{persistent ? "Saved backtest" : "Temporary backtest"}</span><h1>{strategyName}</h1>{config && <p>{config.start_date} to {config.end_date}</p>}<span className={`run-kind ${persistent ? "persistent" : "temporary"}`}>{persistent ? "Saved in Backtests" : "Not saved to Backtests"}</span></div><button className="secondary-button" onClick={onBack}>Back</button></header>
    ...
    {result && <BacktestResultPanel result={result} selectedTimestamp={selectedTimestamp} decisionSessions={sessions} onSelectDecision={(session) => selectDecision(session, true)} />}
    ...
    {run?.status === "succeeded" ? <div ref={analysisRef}><DecisionAnalysis runId={run.id} sessions={sessions} listState={evidenceState} selected={selectedSession} onSelect={(session) => selectDecision(session)} onShowInStrategy={onShowInStrategy ? (componentId) => onShowInStrategy(run.revision_id, componentId) : undefined} /></div> : <section className="analysis-boundary"><div className="analysis-icon">↳</div><div><span className="eyebrow">Analysis</span><h2>Decision analysis is unavailable</h2><p>{run ? "This run did not complete with a result and decision evidence." : "Save this strategy and create a saved Backtest to inspect its decisions."}</p></div></section>}
  </section>;
}
