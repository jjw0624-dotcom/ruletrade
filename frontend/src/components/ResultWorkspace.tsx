...
export function ResultWorkspace({ response, run, strategyName, onBack, onShowInStrategy }: Props) {
  ...
  return <section className="result-workspace page"><header className="workspace-heading"><div><span className="eyebrow">{persistent ? "Saved backtest" : "Temporary backtest"}</span><h1>{strategyName}</h1>{config && <p>{config.start_date} to {config.end_date}</p>}<span className={`run-kind ${persistent ? "persistent" : "temporary"}`}>{persistent ? "Saved in Backtests" : "Not saved to Backtests"}</span></div><button className="secondary-button" onClick={onBack}>Back</button></header>
    ...
    {result && <BacktestResultPanel result={result} selectedTimestamp={selectedTimestamp} />}
    ...
    {run?.status === "succeeded" ? <DecisionAnalysis runId={run.id} onTimestamp={setSelectedTimestamp} onShowInStrategy={onShowInStrategy ? (componentId) => onShowInStrategy(run.revision_id, componentId) : undefined} /> : <section className="analysis-boundary"><div className="analysis-icon">↳</div><div><span className="eyebrow">Analysis</span><h2>Decision analysis is unavailable</h2><p>{run ? "This run did not complete with a result and decision evidence." : "Save this strategy and create a saved Backtest to inspect its decisions."}</p></div></section>}
  </section>;
}
