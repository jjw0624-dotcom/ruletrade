...
import { assetOutcomes, assetPath, type PathStatus } from "../domain/decisionPresentation";
import { pageForDecision, resultEventPage, RESULT_EVENT_PAGE_SIZE, type ResultEventPresentation } from "../domain/resultEvents";
...
export function DecisionAnalysis({ runId, events, allEventCount, listState, selected, selectedAsset, onSelect, onSelectAsset, onShowInStrategy, onViewInFlow, onComparisonReady }: { runId: string; events: ResultEventPresentation[]; allEventCount: number; listState: "loading" | "loaded" | "error"; selected: ResultEventPresentation | null; selectedAsset?: string | null; onSelect: (event: ResultEventPresentation) => void; onSelectAsset?: (asset: string) => void; onShowInStrategy?: ShowRule; onViewInFlow?: ShowRule; onComparisonReady?: (comparison: ComparisonRecord, context: ResearchContext) => void }) {
  ...
  const pageCount = Math.max(1, Math.ceil(events.length / RESULT_EVENT_PAGE_SIZE));
  const visibleEvents = useMemo(() => resultEventPage(events, page), [events, page]);
  useEffect(() => { setPage(pageForDecision(events, selected?.decisionId ?? null)); }, [events, selected?.decisionId]);
  useEffect(() => {
    setWhatIf(null);
    if (!selected) { setDetails([]); setDetailState("idle"); return; }
    let cancelled = false; setDetailState("loading"); setDetails([]);
    Promise.all(selected.decisionIds.map((id) => decisionEvidenceApi.get(runId, id))).then((items) => { if (!cancelled) { setDetails(items.sort((a, b) => a.ordinal - b.ordinal)); setDetailState("loaded"); } }).catch(() => { if (!cancelled) setDetailState("error"); });
  ...
  if (!allEventCount) return <section className="analysis-workspace" aria-label="Decision analysis"><div className="analysis-empty"><h2>Decision details are not available for this older test</h2><p>The saved result is unchanged. RuleTrade will not invent or rerun missing evidence.</p></div></section>;
  if (!events.length) return <section className="analysis-workspace" aria-label="Decision analysis"><div className="analysis-empty"><h2>No Result events match this view</h2><p>The saved Result and Evidence are unchanged. Choose another factual event filter.</p></div></section>;
  return <section className="analysis-workspace" aria-label="Decision analysis"><header className="analysis-heading"><span className="eyebrow">Decisions</span><h2>What happened, and why?</h2><p>Decision-time Evidence contains only information available when the Strategy acted.</p></header><div className="research-layout"><aside className="decision-timeline" aria-label="Decision timeline"><div className="decision-timeline-heading"><h3>Result events</h3><small>{events.length} persisted</small></div>{visibleEvents.map((event) => <button key={event.key} className={`timeline-event ${event.category}${selected?.decisionId === event.decisionId ? " selected" : ""}`} onClick={() => onSelect(event)} aria-pressed={selected?.decisionId === event.decisionId} data-decision-id={event.decisionId}><time>{day(event.sessionId)}</time><strong>{event.label}</strong><small>{event.category}</small></button>)}{pageCount > 1 && <div className="event-pagination"><button disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}>Previous</button><span>{page + 1} / {pageCount}</span><button disabled={page >= pageCount - 1} onClick={() => setPage((value) => Math.min(pageCount - 1, value + 1))}>Next</button></div>}</aside><div className="research-inspector">{!selected && <div className="inspector-prompt"><h3>Select a Result event</h3><p>Use a chart marker or the compact Decision list.</p></div>}{selected && detailState === "loading" && <p role="status">Opening persisted Decision {selected.decisionId.slice(0, 8)}…</p>}{selected && detailState === "error" && <div role="alert"><h3>We couldn't open this persisted Decision</h3><p>The Result remains available. No Test was rerun.</p></div>}{selected && detailState === "loaded" && <Inspector key={selected.decisionId} date={selected.sessionId} details={details} initialAsset={selectedAsset} onSelectAsset={onSelectAsset} onShowInStrategy={onShowInStrategy} onViewInFlow={onViewInFlow} onTryChange={onComparisonReady ? setWhatIf : undefined} />}</div></div>{whatIf && selected && <WhatIfEditor runId={runId} intent={whatIf} context={{ runId, decisionId: selected.decisionId, sessionId: selected.sessionId, asset: whatIf.asset }} onCancel={() => setWhatIf(null)} onComparisonReady={onComparisonReady!} />}</section>;
}

export function Inspector({ date, details, initialAsset, onSelectAsset, onShowInStrategy, onViewInFlow, onTryChange }: { date?: string; details: DecisionEventDetail[]; initialAsset?: string | null; onSelectAsset?: (asset: string) => void; onShowInStrategy?: ShowRule; onViewInFlow?: ShowRule; onTryChange?: (intent: CandidateIntent) => void }) {
  ...
  return <><DecisionSummary date={date} details={details} />{outcomes.length > 0 && <section className="inspector-section asset-hero"><span className="eyebrow">At the decision</span><div className="asset-overview">{outcomes.map((item) => { const observed=observedValue(item.asset,details); return <button key={item.asset} className={`asset-outcome ${item.kind}`} onClick={() => { setAsset(item.asset); onSelectAsset?.(item.asset); }} aria-pressed={asset === item.asset}><StatusIcon status={statusForOutcome(item.kind)} /><span><strong>{item.asset}</strong><small>{observed}{observed ? " · " : ""}{item.label}</small></span></button>; })}</div>{asset && <AssetExplanation asset={asset} details={details} onShowInStrategy={onShowInStrategy} onViewInFlow={onViewInFlow} onTryChange={onTryChange} />}</section>}<SelectionPath details={details} selectedAsset={asset} onSelectAsset={(next) => { setAsset(next); onSelectAsset?.(next); }} onShowInStrategy={onShowInStrategy} /><Portfolio details={details} onShowInStrategy={onShowInStrategy} /><MoreDetails details={details} onShowInStrategy={onShowInStrategy} /></>;
}
...
function DecisionSummary({ date, details }: { date?: string; details: DecisionEventDetail[] }) {
  ...
  return <header className="decision-summary"><span className="eyebrow">What happened</span><time>{day(date)}</time><h3>{sentence}</h3>{portfolio && <p><span>Final portfolio</span><strong>{portfolio}</strong></p>}</header>;
}

export function AssetExplanation({ asset, details, onShowInStrategy, onViewInFlow, onTryChange }: { asset: string; details: DecisionEventDetail[]; onShowInStrategy?: ShowRule; onViewInFlow?: ShowRule; onTryChange?: (intent: CandidateIntent) => void }) {
