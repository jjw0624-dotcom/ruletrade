...
export function DecisionAnalysis({ runId, sessions, listState, selected, selectedAsset, onSelect, onSelectAsset, onShowInStrategy }: { runId: string; sessions: DecisionSession[]; listState: "loading" | "loaded" | "error"; selected: DecisionSession | null; selectedAsset?: string | null; onSelect: (session: DecisionSession) => void; onSelectAsset?: (asset: string) => void; onShowInStrategy?: (componentId: string, fieldPath: string | null | undefined, asset: string | null) => void }) {
  ...
  return <section className="analysis-workspace" aria-label="Decision analysis"><header className="analysis-heading"><span className="eyebrow">Analysis</span><h2>Explore this strategy's decisions</h2><p>Choose a chart marker or browse the timeline. Both open the same decision context.</p></header><div className="research-layout"><aside className="decision-timeline" aria-label="Decision timeline"><h3>Decision timeline</h3>{sessions.map((session) => <button key={session.sessionId} className={selected?.sessionId === session.sessionId ? "timeline-event selected" : "timeline-event"} onClick={() => onSelect(session)} aria-pressed={selected?.sessionId === session.sessionId}><time>{day(session.sessionId)}</time><strong>{session.label}</strong><small>{session.events.length} recorded step{session.events.length === 1 ? "" : "s"}</small></button>)}</aside><div className="research-inspector">{!selected && <div className="inspector-prompt"><h3>Select a decision</h3><p>Use a marker on the chart or a date in the timeline.</p></div>}{selected && detailState === "loading" && <p role="status">Opening this decision…</p>}{selected && detailState === "error" && <div role="alert"><h3>We couldn't open this decision</h3><p>The timeline evidence remains unchanged.</p></div>}{selected && detailState === "loaded" && <Inspector key={selected.sessionId} details={details} initialAsset={selectedAsset} onSelectAsset={onSelectAsset} onShowInStrategy={onShowInStrategy} />}</div></div></section>;
}

export function Inspector({ details, initialAsset, onSelectAsset, onShowInStrategy }: { details: DecisionEventDetail[]; initialAsset?: string | null; onSelectAsset?: (asset: string) => void; onShowInStrategy?: (componentId: string, fieldPath: string | null | undefined, asset: string | null) => void }) {
  const outcomes = assetOutcomes(details);
  const preferredAsset = outcomes.find((item) => item.kind === "failed")?.asset ?? outcomes.find((item) => item.kind === "blocked")?.asset ?? outcomes.find((item) => item.kind === "ranked_out")?.asset;
  const [asset, setAsset] = useState(initialAsset && outcomes.some((item) => item.asset === initialAsset) ? initialAsset : preferredAsset ?? outcomes[0]?.asset ?? null);
  const refs = dedupeRefs(details.flatMap((item) => item.source_components));
  return <><section className="inspector-section"><span className="eyebrow">What happened?</span><h3>{happenedText(details)}</h3></section><Why details={details} />{outcomes.length > 0 && <section className="inspector-section"><span className="eyebrow">Asset outcomes</span><h3>What happened to each asset?</h3><div className="asset-overview">{outcomes.map((item) => <button key={item.asset} className={`asset-outcome ${item.kind}`} onClick={() => { setAsset(item.asset); onSelectAsset?.(item.asset); }} aria-pressed={asset === item.asset}><StatusIcon status={statusForOutcome(item.kind)} /><span><strong>{item.asset}</strong><small>{item.label}</small></span></button>)}</div>{asset && <AssetExplanation asset={asset} details={details} onShowInStrategy={onShowInStrategy} />}</section>}<Funnel details={details} /><Sleeves details={details} /><Snapshots details={details} /><section className="inspector-section"><span className="eyebrow">Where did this come from?</span><h3>Strategy rules recorded for this decision</h3><div className="source-list">{refs.map((ref) => <div key={`${ref.role}-${ref.component_id}-${ref.field_path ?? "component"}`}><span>{friendlyRole(ref.role)}</span>{onShowInStrategy && <button className="secondary-button" onClick={() => onShowInStrategy(ref.component_id, ref.field_path, asset)}>View rule</button>}</div>)}</div></section><details className="evidence-details"><summary>Technical evidence details</summary>{details.map((item) => <div key={item.id}><b>#{item.ordinal} · {item.evidence.kind.replaceAll("_", " ")}</b><span>{item.phase.replaceAll("_", " ")}</span></div>)}</details></>;
}
...
export function AssetExplanation({ asset, details, onShowInStrategy }: { asset: string; details: DecisionEventDetail[]; onShowInStrategy?: (componentId: string, fieldPath: string | null | undefined, asset: string | null) => void }) {
...
function statusForOutcome(kind: ReturnType<typeof assetOutcomes>[number]["kind"]): PathStatus { return kind === "selected" ? "passed" : kind === "failed" || kind === "blocked" ? "failed" : kind === "fallback" || kind === "replaced" ? "fallback" : "neutral"; }
...
function Funnel({ details }: { details: DecisionEventDetail[] }) {
  ...
  if (selection?.kind === "selection") { stages.push(["Ranked", String(selection.ranked.length)]); stages.push(["Candidates", "required_count" in selection ? `${selection.candidates.length} / ${selection.required_count}` : String(selection.candidates.length)]); stages.push(["Primary selected", String(selection.primary_selected.length)]); }
