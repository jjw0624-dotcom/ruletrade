...
export function Inspector({ details, onShowInStrategy }: { details: DecisionEventDetail[]; onShowInStrategy?: (componentId: string) => void }) {
  ...
  const initialAsset = outcomes.find((item) => item.kind === "failed")?.asset ?? outcomes.find((item) => item.kind === "blocked")?.asset ?? outcomes.find((item) => item.kind === "ranked_out")?.asset;
