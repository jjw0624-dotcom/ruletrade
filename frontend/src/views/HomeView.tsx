...
export function HomeView({ status, strategies, error, onOpen, onRetry, onCreate, onExample }: { status: Status; strategies: StrategyRecord[]; error: string | null; onOpen: (id: string) => void; onRetry: () => void; onCreate: () => void; onExample: (id: ExampleId) => void }) {
  const recent = [...strategies].sort((a, b) => b.updated_at.localeCompare(a.updated_at));
  return <section className="page home-page"><header className="home-heading"><div><span className="eyebrow">Home</span><h1>What were you working on?</h1></div><button className="primary-button" onClick={onCreate}>+ New strategy</button></header>
