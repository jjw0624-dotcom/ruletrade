...
export function StrategiesView({ status, strategies, error, onExplore, onOpen, onRetry }: {
  ...
  return <section className="page strategies-page"><header className="page-heading"><span className="eyebrow">Strategies</span><h1>My Strategies</h1><p>Saved strategies and their latest revisions.</p><button className="primary-button" onClick={onExplore}>+ New strategy</button></header>
    ...
    {status === "loaded" && strategies.length === 0 && <div className="empty-state"><div className="empty-icon">＋</div><h2>No saved strategies yet</h2><p>Start with a backend-supported example, then make it yours.</p><button className="primary-button" onClick={onExplore}>Explore examples</button></div>}
