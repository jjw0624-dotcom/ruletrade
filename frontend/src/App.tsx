...
export default function App() {
  ...
  return <AppShell route={route} strategyName={strategyName} navigate={navigate}>
    ...
    {(route.page === "example" || route.page === "strategy") && workspace && workspaceStatus === "loaded" && <StrategyEditorProvider key={workspace.detail?.current_revision.id ?? workspace.example.id} bootstrap={workspace.bootstrap}><StrategyEditor example={workspace.example} persisted={workspace.detail} confirmation={adoptionNotice} onDirtyChange={setDirty} onArchived={() => navigate({ page: "strategies" })} onOpenRun={(runId) => navigate({ page: "run", runId })} sourceFocus={sourceFocus} onBackToResearch={sourceFocus?.returnRoute ? () => navigate(sourceFocus.returnRoute!) : undefined} backToResearchLabel={sourceFocus?.researchContext ? `Back to ${new Date(`${sourceFocus.researchContext.sessionId}T00:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" })}${sourceFocus.researchContext.asset ? ` · ${sourceFocus.researchContext.asset}` : ""}` : undefined} /></StrategyEditorProvider>}
