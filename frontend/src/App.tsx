...
export default function App() {
  ...
  return <AppShell route={route} strategyName={strategyName} navigate={navigate}>
    ...
    {(route.page === "example" || route.page === "strategy") && workspace && workspaceStatus === "loaded" && <StrategyEditorProvider key={workspace.detail?.current_revision.id ?? workspace.example.id} bootstrap={workspace.bootstrap}><StrategyEditor example={workspace.example} persisted={workspace.detail} onDirtyChange={setDirty} onArchived={() => navigate({ page: "strategies" })} /></StrategyEditorProvider>}
