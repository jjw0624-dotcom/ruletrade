...
export async function loadEditorBootstrap(example: string): Promise<EditorBootstrap> {
  const response = await fetch(`/api/v1/editor/bootstrap?example=${encodeURIComponent(example)}`);
