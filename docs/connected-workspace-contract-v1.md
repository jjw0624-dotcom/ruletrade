...
**CURRENT audit, based on the merged editable-representations work from PR #17.** This document describes concrete code boundaries; it does not add future modes or a universal representation model. The contract was originally reviewed as stacked PR #18 and is now replayed onto `main` without its stale branch ancestry.
